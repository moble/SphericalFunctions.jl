                ...
                for n in 0:ℓₘₐₓ
                    for m′ in -n:n
                        for m in -n:n
                            ...
                            d_recurrence = d[Deprecated.WignerDindex(n, m′, m)]
...
@testitem "Test d signatures" setup=[Utilities] begin
    ...
    @testset "$T" for T in [BigFloat, Float64, Float32]
        ...
        for β in βrange(T)
            expiβ = cis(β)
            dA = Deprecated.d_matrices(β, ℓₘₐₓ)  # 1
            dB = Deprecated.d_matrices(expiβ, ℓₘₐₓ)  # 2
            @test array_equal(dA, dB)
            dB .= 0
            Deprecated.d_matrices!(dB, β, ℓₘₐₓ)  # 5
            @test array_equal(dA, dB)
            dB .= 0
            Deprecated.d_matrices!(dB, expiβ, ℓₘₐₓ)  # 6
            @test array_equal(dA, dB)
            d_storage = Deprecated.d_prep(ℓₘₐₓ, T)
            dB = Deprecated.d_matrices!(d_storage, β)  # 3
            @test array_equal(dA, dB)
            dB .= 0
            dB = Deprecated.d_matrices!(d_storage, expiβ)  # 4
            @test array_equal(dA, dB)
        end
    end
end
