        ...
        for s ∈ -3:3
            for ℓₘᵢₙ ∈ 0:abs(s)+2
                for ℓₘₐₓ ∈ [7,8]
                    ...
                    #     # R₋ = Rₓ + i Rᵧ
                    #     # R₊ = Rₓ - i Rᵧ
                    #     # R₊R₋ = (Rₓ - i Rᵧ)(Rₓ + i Rᵧ) = Rₓ² + Rᵧ² + i[Rₓ,Rᵧ] = R² - Rz² + i[Rₓ,Rᵧ]
                    #     # R₋R₊ = (Rₓ + i Rᵧ)(Rₓ - i Rᵧ) = Rₓ² + Rᵧ² - i[Rₓ,Rᵧ] = R² - Rz² - i[Rₓ,Rᵧ]
                    #     # R² = (2Rz² + R₊R₋ + R₋R₊)/2
                    #     R1 = R²(s, ℓₘᵢₙ, ℓₘₐₓ, T)
                    #     R2 = T.(Array(
            ...
            for s in -2:2
                let ℓₘᵢₙ = 0
                    ...
                    let
                        ...
                        # # [ð, ð̄] = -2Rz
                        # explain(ð(s-1, ℓₘᵢₙ, ℓₘₐₓ, T)*ð̄(s, ℓₘᵢₙ, ℓₘₐₓ, T)
                        # - ð̄(s+1, ℓₘᵢₙ, ℓₘₐₓ, T)*ð(s, ℓₘᵢₙ, ℓₘₐₓ, T),
                        # -2Rz(s, ℓₘᵢₙ, ℓₘₐₓ, T), "[ð,ð̄]($s, $ℓ, $m)", T, s, ℓₘᵢₙ, ℓₘₐₓ, ϵ)
                        # @test (
