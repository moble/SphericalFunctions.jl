...
```math
σ = \mathrm{sgn}(m)\, \mathrm{sgn}(m'),
\qquad \mathrm{sgn}(0) = +1.
```

For integer indices ``σ ≡ 1``, ``ϵ_{|m'|} = (-1)^{m'}``, and ``(-1)^{ℓ-m} = (-1)^{ℓ+m}``, so
the first two relations reduce to plain symmetry and the last two to the forms usually
quoted, ``(-1)^{ℓ+m+m'}`` and ``(-1)^{m+m'}``.  For half-integer indices ``σ`` is genuinely
``-1`` whenever ``\mathrm{sgn}(m) ≠ \mathrm{sgn}(m')`` — for example
``H^{1/2}_{1/2,-1/2} = -\sin(β/2)`` while ``H^{1/2}_{-1/2,1/2} = +\sin(β/2)`` — and
``ℓ+m+m'`` is not even an integer, so the exponents above have to be written in the forms
``ℓ-m`` and ``m'-m``, which are.

!!! warning "Do not hand-roll these symmetries"

    In the code, ``σ`` is applied for you by
...
## Steps to compute ``H`` (redesigned)

The following describes various details that are not spelled out
correctly by [Gumerov_2015](@citet).  All equation numbers refer to
...
!!! note "Half-integer indices"
    ...
    The steps above describe the integer ladder.  For half-integer ``ℓ, m', m`` the shape of
    the computation is the same, with three differences.  Step 3 — which climbs from the
    ``m' = 0`` axis to the ``m' = 1`` row — has no half-integer counterpart, because there is
    no ``m' = 0`` row; it is replaced by a seed that produces *both* rows ``m' = \pm 1/2``
    at once from the integer axes ``H^{j}_{0,m}`` and ``H^{j+1}_{0,m}`` with ``j = ℓ - 1/2``.
    Steps 4 and 5 are then the same recurrences, but start from ``m' = 1/2`` and
    ``m' = -1/2`` rather than ``m' = 1`` and ``m' = 0``; both are written in terms of
    ``m' ≥ 1 - ℓ_\mathrm{min}`` and ``m' ≤ -ℓ_\mathrm{min}``, which specialize to the
    integer bounds when ``ℓ_\mathrm{min} = 0``.  And step 6 is never run at all: the batched
    engine leaves the wedge alone and applies the symmetries (with their sign ``σ``) on the
    fly, as each element is read.


Compute values ``H^{0,m}_{n}(β)`` for ``m=0,\ldots,n`` and
``H^{0,m}_{n+1}(β)`` for ``m=0,\ldots,n+1``.  Using Eq. (32), we see
that within Gumerov and Duraiswami's conventions
```math
\begin{aligned}
...
Every index this recursion touches is sequential for a single rotation: each ``ℓ`` is built
from the one before, and within a given ``ℓ`` the ladders of steps 4 and 5 read the rows that
the previous iteration has just written.  The rotation itself is the one free dimension — the
same arithmetic on unrelated data — which is why the implementation runs a whole batch of
rotors through every step at once, with the rotor index varying fastest in memory so that the
innermost loop is the one that can be vectorized.  What that means for the interface is
described under "Reusing the workspace" on the [Wigner matrices](@ref
