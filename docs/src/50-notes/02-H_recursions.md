...
!!! note "Superseded"
    ...
    This page describes the ``H`` recursion as it was implemented before version 3.0, in the
    whole-array form of `src/deprecated/Hrecursion.jl` (deleted in 3.0).  The algorithm the
    package runs now — batched over rotors, storing only the wedge, and extended to
    half-integer ``ℓ`` — is described on
    [the redesigned page](@ref "Algorithm for computing ``H`` (redesigned)"), which is the
    one to read.  This page is kept for the derivations it records.
