...
corresponding derivation.  The [comparisons](@ref "Comparisons")
section at the bottom of this page relates these conventions to the
literature and to other software; each source is treated on its own
page, collected in the sidebar under **Conventions → Comparisons**.
...
unit basis vectors ``(𝐱, 𝐲, 𝐳)``.  [Details.](@ref
conv_three_dimensional_space)

## [Spherical coordinates](@id summary_spherical_coordinates)
We define spherical coordinates ``(r, θ, ϕ)`` and unit basis vectors
``(𝐧, \boldsymbol{θ}, \boldsymbol{ϕ})`` in the standard "physics"
convention:
```math
x = r \sin θ \cos ϕ,
\qquad
...
𝐢𝐣𝐤 = -1.
```
In the code, this quaternion is represented by the components ``(W, X,
Y, Z)``, in that order.  The conjugate is ``𝐐̄ = W-X𝐢-Y𝐣-Z𝐤``, the
norm is ``\|𝐐\| = \sqrt{W^2+X^2+Y^2+Z^2}``, and for a unit
"pure-vector" quaternion ``𝐮`` we have ``\exp(𝐮\,θ) = \cos θ + 𝐮
\sin θ``.
...
or equivalently
```math
𝐑_{ϕ, θ, γ}\, \frac{𝐱 + i 𝐲}{\sqrt{2}}\, 𝐑_{ϕ, θ, γ}^{-1} = e^{-iγ}\, 𝐦.
```
Note that the factor ``e^{-iγ}`` in the latter form identifies ``γ``
as the angle related to spin, which is responsible for rotating the
tangent basis — conventionally identified in terms of the complex dyad
``𝐦``.  Comparing with the definition of spin weight
[below](@ref summary_spin_weight), it says that the transported dyad
has spin weight ``1``.
[Details.](@ref conv_euler_angles)
...
Varshalovich et al., and several other important references; other
references typically have a relative conjugate, transpose, and/or
sign.  See below for the full [Table of comparisons](@ref).
...
some of these.  Note that there is no single page containing all the
comparisons; rather, *each source gets its own page*.  Those pages are
collected in the sidebar under **Conventions → Comparisons**, and are
also linked individually from the table at the end of this section.
Each one quotes the source's actual equations, implements them in
Julia, and tests them numerically against this package.  They are
written with
...
Each row below links to that source's own comparison page.

| Reference | Verdict |
|-----------|---------|
| [Blanchet (2024)](@ref "Blanchet (2024)") | Spin-weight ``-2`` spherical harmonics agree with ours.  His ``d^{ℓm}`` agrees with our ``d^{(ℓ)}_{m,2}``. |
...
| [Tait (1867 and 1868)](@ref "Tait (1867 and 1868)") | Right-handed basis and multiplication table as ours; Euler angles ``(ψ, θ, ϕ)`` in ``z-y'-z''`` form are exactly our ``(α, β, γ) = (ψ, θ, ϕ)`` — equivalently, the fixed-axis rotations applied in the order ``ϕ, θ, ψ`` — and his quaternion matches ours. |
