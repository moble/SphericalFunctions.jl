...
!!! info "Summary"
    Blanchet's definition of the spin-weight ``-2`` spherical harmonics agrees with the
    definition used in the `SphericalFunctions` package.  His ``d^{ℓm}`` is not a full
    Wigner ``d`` matrix — it has only two labels — but is precisely the single column
    ``d^{(ℓ)}_{m,2}`` in this package's conventions.
...
#
# Blanchet's ``d^{ℓm}`` is not a Wigner ``d`` matrix: it has only the two labels ``(ℓ, m)``,
# because it is only used to express ``Y^{ℓ,m}_{-2}``.  Comparing his Eq. (184a) with our
# own expression for the spin-weighted harmonics in spherical coordinates,
# ```math
