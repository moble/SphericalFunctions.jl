...
# These successful tests show that Wikipedia's ``d``, ``D``, ``Y_ℓ^m``, and ``{}_sY_{ℓm}``
# all agree with the corresponding functions defined by the `SphericalFunctions` package.
