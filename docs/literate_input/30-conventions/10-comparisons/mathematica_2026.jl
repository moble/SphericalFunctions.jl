...
2026-09-08.  (`WignerD` was introduced in Version 8.0, 2010; `SphericalHarmonicY` in Version
1.0, 1988.)
...
> For ``ℓ \geq 0``, ``Y_ℓ^m(θ, ϕ) = \sqrt{(2ℓ+1)/(4π)} \sqrt{(ℓ-m)! / (ℓ+m)!} P_ℓ^m(\cos θ)
> e^{imϕ}`` where ``P_ℓ^m`` is the associated Legendre function,

and the [`LegendreP`](@cite Mathematica_LegendreP) documentation defines that function (for
its default "type 1") as
...
The Condon–Shortley phase is thus included in `LegendreP`, and we expect agreement with [our
spherical harmonics](@ref summary_spherical_harmonics).  For negative ``m``, the
documentation does not spell out the continuation of ``P_n^m``; we use the standard relation
for Legendre functions with this phase convention (which is what Mathematica evaluates to),
[DLMF 14.9.3](https://dlmf.nist.gov/14.9#E3),
```math
...
(The two-argument form `WignerD[{j, m1, m2}, θ, ϕ]` is `WignerD[{j, m1, m2}, 0, θ, ϕ]`.) The
phases ``e^{+im_1ψ}`` and ``e^{+im_2ϕ}`` are the complex conjugates of [ours](@ref
summary_wigner_D), so Mathematica's ``D`` is certainly not equal to our ``𝔇``.  There are
two natural candidates for the relation: the plain complex conjugate,
...
\overline{𝔇_{m,0}(ϕ, θ, 0)} = e^{imϕ} d_{m,0}(θ)``, whereas ``\overline{𝔇_{0,m}(0, θ, ϕ)}
= e^{imϕ} d_{0,m}(θ) = (-1)^m e^{imϕ} d_{m,0}(θ)``; and the explicit example gives ``-\sin θ
/ \sqrt{2}`` where ``d_{0,1}(θ) = +\sin θ/\sqrt{2}`` but ``d_{1,0}(θ) = -\sin θ/\sqrt{2}``.
So the plain conjugate violates both, and the second candidate satisfies both.  We therefore
take
```math
...
this is as far as the documentation allows us to go: the identities determine the ``m_1 =
0`` row and ``m_2 = 0`` column of Mathematica's ``D`` completely, but not the general
element.
...
# satisfied by ``𝔇^{(j)}_{-m_1,-m_2}(ψ, θ, ϕ) =
# (-1)^{m_1-m_2}\,\overline{𝔇^{(j)}_{m_1,m_2}(ψ, θ, ϕ)}``.
