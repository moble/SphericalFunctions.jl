...
```
for a positive normalization constant that does not depend on ``m``.  This is equivalent to    
```math
Y_{l,m}(ϑ, ϕ)
=
\mathrm{const}\, (-1)^m\, e^{imϕ}\, 𝔇^{(l)}(ϕ, ϑ, 0)_{m0}.
```
Note the factor ``(-1)^m``, which does not appear in [our relation](@ref
summary_spherical_harmonics) ``Y_{ℓ,m} = \sqrt{(2ℓ+1)/4π}\, \overline{𝔇_{m,0}}``.  In fact,
with the relation between Wigner's ``𝔇`` and ours given above, ``(-1)^m 𝔇^{(ℓ)}(ϕ, θ,
0)_{m0}|_{\text{Wigner}} = \overline{𝔇_{m,0}(ϕ, θ, 0)}``, so the two relations are the same
and Wigner's spherical harmonics are the standard ones.  We verify this as well.
