@testmodule ConventionsUtilities begin
    ...
    # Reference implementations of *this package's settled conventions*, as documented on
    # the conventions "Summary" page.  Every comparison page compares the literature against
    # these functions, so that the pages state relations to the settled conventions rather
    # than to whatever the code happens to compute today.
    #
    # `D`, `d` and `Y` are the package's own v3 functions, which implement the settled
    # conventions 𝔇ˡₘ′ₘ(α, β, γ) = exp(-𝒾 m′ α) dˡₘ′ₘ(β) exp(-𝒾 m γ) and ₛYₗₘ = (-1)^s
    # √((2ℓ+1)/4π) conj(𝔇ˡₘ,₋ₛ); the "reference conventions" testitem below pins them to
