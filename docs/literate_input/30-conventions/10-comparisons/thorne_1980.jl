...
gravitational-wave literature.  Thorne uses the standard (physicists') spherical coordinates
``(θ, ϕ)``, and writes the indices of the spherical harmonics as superscripts: ``Y^{ℓ m}``.
