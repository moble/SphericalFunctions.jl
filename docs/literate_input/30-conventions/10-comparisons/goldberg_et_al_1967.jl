...
!!! info "Summary"
    Goldberg et al. use the same conventions as we do for spherical coordinates and Euler
    angles.  Their Wigner ``D`` matrix is defined as a function of the *inverse* rotation,
    and is related to ours by
    ```math
...
page](@ref "Newman-Penrose (1966)")), and the first to relate them to the Wigner D-matrices.
They also used functional symmetry to introduce what we recognize geometrically as the
right-Lie derivative operator.

## Coordinates and derivative operators

Goldberg et al. start out by saying that they work in "three-dimensional Euclidean space
with polar  coordinates ``r, θ, ϕ``".  They write down Newman-Penrose's operator acting on a
field of spin weight ``s`` as
```math
...
They then "define a rotation ``R(α, β, γ)`` of Euler angles ``α, β, γ`` as being composed of
``γ`` about ``OZ`` followed by ``β`` about ``OY`` and then ``α`` about ``OZ``", noting that
"this procedure is clearly equivalent to the more usual one of a  rotation ``γ`` around
``OZ``, followed by ``β`` around ``OY'`` and finally ``α`` around ``OZ''``."  This agrees
precisely with our definition.

They go on to define operators "familiar from the theory of the symmetric top".
Unfortunately, this section contains a small mess of sign errors.  Their *corrected* Eqs.
(3.16) read
```math
...
\right),
```
where the correction is to flip the sign of that last term in parentheses.  With that
correction, we have perfect agreement between [this package's ``L`` operators in Euler
coordinates](@ref euler_L_S3) and Goldberg's.

They then construct a second set of operators by swapping ``α`` and ``-γ``, though even that
statement has a typo in the original, and the swapping is done incorrectly.  Their
*corrected* Eqs. (3.18) read
```math
...
It's also worth noting that there is an incorrect factor of ``i`` in the upper line of Eq.
(3.20), but the second line is correct.  They then find the relationship between ``K_+`` and
``\eth`` — but have yet another sign error.  Their *corrected* [Eq. (3.21)] reads
```math
[K_+ D^{l}_{-sm}]_{\alpha=\phi, \beta=\theta, \gamma=0}
=
-\eth D^{l}_{-sm} (\phi\theta 0),
```
where the correction is the sign on the right-hand side.  Including this corrected sign, and
remembering that ``K_+ = -R_+``, we see that this agrees with [this package's relationship
``R_+ = \eth``](@ref summary_spin_weight).
...
```
and the action of the Newman–Penrose operators, with the minus sign in the lowering relation
[Eqs. (2.7a) and (2.7b)]
```math
\begin{gather}
...
If we relate two vectors by a rotation matrix as ``x'^k = R^{kl} x^l``, then Goldberg et al.
define ``D`` by its action on spherical harmonics [Eq. (3.3)]
```math
...
the first and last Euler angles swapped — as expected from a definition in terms of the
inverse rotation ``R(αβγ)^{-1} = R(-γ, -β, -α)``.
