...
e^{imϕ} P_{ℓ}^{m}(\cos θ),
```
where ``ϵ = (-1)^m`` for ``m\geq 0`` and ``ϵ = 1`` for ``m\leq 0``.  Since ``P_ℓ^m`` depends
only on ``|m|``, this places the Condon–Shortley phase entirely in ``ϵ``; the result is the
standard Condon–Shortley convention, so we expect agreement with [ours](@ref
