...
define.  The method we use here is as direct and explicit as possible.  We will quote them
and implement them below.  In particular, Condon and Shortley provide a formula for the φ=0
part in terms of iterated derivatives of a power of sin(θ).  Rather than expressing these
derivatives in terms of the Legendre polynomials — which would subject us to another round
of ambiguity — we will use automatic differentiation to compute the derivatives explicitly.

Condon and Shortley are not very explicit about the meaning of the spherical coordinates,
but they do describe them as "spherical polar coordinates ``r, θ, φ``".  Immediately before
equation (1) of section 4³ (page 50), they define the angular-momentum operator
```math
