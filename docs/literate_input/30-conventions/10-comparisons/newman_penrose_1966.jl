...
They are a little ambiguous about the relationship of the complex basis vector ``m^\mu`` to
the coordinates:
...
they mean that the components are *positive* multiples of ``\partial_θ`` and ``\partial_ϕ``
respectively, in which case we have
```math
...
\eta' = e^{i s ψ} \eta.
```
Raising the spherical coordinates ``(θ, ϕ)`` to Euler angles ``(ϕ, θ, -ψ)``, we see that the
rotor ``𝐑_{ϕ, θ, -ψ}`` rotates the ``𝐳`` basis vector to the point ``(θ, ϕ)``, and it
...
detail on the [calculation page](@ref euler_R_S2) — we show that these are precisely ``\eth
= R_+ = R_x + i R_y`` and ``\bar{\eth} = -R_- = -(R_x - i R_y)`` in terms of our right
angular-momentum operators, so that
```math
...
# ``\bar{\eth}\eth`` test below.  `cis` computes `Complex(cos, sin)` unconditionally, and so
# differentiates correctly to all orders.
...
# Finally, we compare Newman and Penrose's ``ζ``-expression to the package.  Since they only
# give it up to proportionality, we determine the constant of proportionality — which turns
# out to be ``(-1)^ℓ \sqrt{4π / [(2ℓ+1)(ℓ+m)!(ℓ-m)!]}`` — and assert it exactly.  (Newman
# and Penrose encapsulate this as ``a_{l,m}``, but only say its "exact values are
