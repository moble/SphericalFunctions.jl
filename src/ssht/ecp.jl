...
function SSHTECP(
    ...
    if (message = check_threads()) != ""
        @warn """$message
        Computations with SSHTECP can benefit greatly from using all available threads if many
    ...
    SSHTECP(s, ℓₘₐₓ, θ, Nϕ, quadrature_weights, plans, Gs, Hwedge, H_rec_coeffs)
end


function LinearAlgebra.ldiv!(f̃, ssht::SSHTECP, f)  # Compute `f̃ = ssht \ f`, storing the result in `f̃`
    s1 = size(f̃)
