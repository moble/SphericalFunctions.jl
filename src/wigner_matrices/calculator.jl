...
@generated function a(::Type{W}) where {W<:WignerMatrixCalculator}
    [√((n+1+m) * (n+1-m) / T(W)((2n+1)*(2n+3))) for n in 0:ℓₘₐₓ(W)+1 for m in 0:n]
end
a(w) = a(typeof(w))
@generated function b(::Type{W}) where {W<:WignerMatrixCalculator}
    [(m<0 ? -1 : 1) * √((n-m-1) * (n-m) / T(W)((2n-1)*(2n+1))) for n in 0:ℓₘₐₓ(W)+1 for m in -n:n]
end
b(w) = b(typeof(w))
@generated function d(::Type{W}) where {W<:WignerMatrixCalculator}
